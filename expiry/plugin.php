...
function expiry_add_pages() {
        yourls_register_plugin_page( 'expiry', 'Expiration des liens', 'expiry_do_page' );
}
...
function expiry_do_page() {
	...
	$unique = ( YOURLS_UNIQUE_URLS == true ) ? ' disabled="disabled" <p><strong>Notice:</strong> <code>YOURLS_UNIQUE_URLS</code> est défini à <code>true</code>. Cette valeur doit être définie à <code>false</code> pour utiliser cette fonction.</p>' : ' > Utiliser une URL globale post-expiration ?';
		...
		<div id="tabs">
			...
			<div class="wrap_unfloat">
				<ul id="headers" class="toggle_display stat_tab">
					<li class="selected"><a href="#stat_tab_config"><h2>Configuration</h2></a></li>
					<li><a href="#stat_tab_exp_list"><h2>Liste des expirations</h2></a></li>
					<li><a href="#stat_tab_prune"><h2>Nettoyer</h2></a></li>
				...
				<form method="post">
					...
					<div>
						<select name="expiry_global_expiry" size="1" >
							<option value="click" {$globalExp['click']}>Compteur de clics</option>
							<option value="clock" {$globalExp['clock']}>Compte à rebours</option>
							<option value="none" {$globalExp['none']}>Aucun</option>
						</select>
						<p>Définissez cette option si vous souhaitez une condition d'expiration globale pour TOUS les nouveaux liens.</p>
						<p>Par exemple, vous pouvez faire en sorte que chaque nouveau lien expire dans 3 jours, sauf indication contraire. Définissez à 'Aucun' pour un traitement classique par YOURLS.</p>
					</div>
					<br>
					<h3>Valeur par défaut du compteur de clics</h3>
					...
					<div>
						...
						<p>Si le type d'expiration est défini sur 'clic' et qu'aucune valeur de 'décompte' n'est définie, l'expiration revient à cette valeur.</p>
					</div>
					<br>
					<h3>Durée par défaut du compte à rebours</h3>
					...
					<div>
						...
						<select name="expiry_default_age_mod" >
							...
							<option value="hour" {$ageMod['day']}>Heure(s)</option>
							<option value="day" {$ageMod['hour']}>Jour(s)</option>
						</select>
						<p>Si le type d'expiration est défini sur 'horloge' sans qu'aucune autre condition ne soit définie, l'expiration revient à cette valeur.</p>
					</div>
					<br>
					<h3>Comportement d'interception de l'expiration</h3>
					
					<div>
						<select name="expiry_intercept" size="1" >
							<option value="simple" {$intercept['simple']}>Modèle YOURLS</option>
							<option value="template" {$intercept['template']}>Modèle Bootstrap</option>
							<option value="custome" {$intercept['custome']}>URL définie</option>
						</select>
						<p>Le clic qui provoque l'expiration d'un lien doit être traité, nous l'interceptons. Vous pouvez choisir comment faire cela ici.</p>
					</div>

					<div style="display:$ciVisChk;">
						<br>
						<h3>Page personnalisée interception</h3>

						<div>

							<p><label for="expiry_custom">Entrez l'URL d'interception ici</label> <input type="text" size=40 id="expiry_custom" name="expiry_custom" value="$opt[1]" /></p>
							<p>Si vous définissez l'option ci-dessus sans définir ce paramètre, le comportement par défaut est rétabli.</p>

						</div>
					</div>
					<br>
					<h3>traitement post-expiration</h3>
					...
					<div class="checkbox">
						...
						<p>Au lieu d'être supprimés de la base de données, les liens courts expirés peuvent être transférés vers une nouveau lien. Vous pouvez définir une valeur globale ici.</p>

						<p>Une autre façon d’y parvenir serait d’utiliser le <a href="https://diegopeinador.blogspot.com/2013/04/fallback-url-simple-plugin-for-yourls.html" target="_blank">greffon de "basculement"</a>. Combinés, ils offrent une grande flexibilité.</p>
					</div>

					<div style="display:$gpxVisChk;">
						<br>
						<h3>URL de destination globale après expiration</h3>

						<div>

							<p><label for="expiry_custom">Entrez l'URL d'interception ici</label> <input type="text" size=40 id="expiry_custom" name="expiry_custom" value="$opt[1]" /></p>
							<p>Si vous définissez l'option ci-dessus sans définir cette option, le comportement par défaut est rétabli.</p>

						</div>
					</div>
					
					<br>
					<h3>Les balises d'expiration</h3>

					<div class="checkbox">
						<label>
							<input name="expiry_expose" type="hidden" value="false" />
							<input name="expiry_expose" type="checkbox" value="true" $expChk > Exposer? 
						</label>
						<p>Si cette option est activée, les liens comportant une expiration seront marqués d'une surbrillance verte dans l'interface d'administration.</p>
					</div>

					<br>
					<h3>Gestion de la table d'expiration</h3>

					<div class="checkbox">
						<label>
							<input name="expiry_table_drop" type="hidden" value="false" />
							<input name="expiry_table_drop" type="checkbox" value="true" $tblChk > Abandon des données? 
						</label>
						<p>Si cette option est sélectionnée, les données d'expiration seront vidées si le greffon est désactivé. Ne cochez pas cette case pour préserver ces données.</p>
					</div>
			...
			<div  id="stat_tab_exp_list" class="tab">
	 			...
	 			<h3>Liste des URL d'expiration</h3>
				<h4>Vous pouvez ajouter une condition d'expiration à un lien déjà présent dans la base de données.</h4>
...
echo <<<HTML
			...
			<div  id="stat_tab_prune" class="tab">
	 			...
	 			<h3>Avancé: maintenance de la base de données </h3>

				<p>Il y a 3 paramètres pour cette opération</p>
				<ul>
					<li><strong>Expiré</strong>: Localiser les liens ayant dépassé leur expiration, et les traiter.</li>
					<li><strong>Scrub</strong>: Copier toutes les données expirées. les URL sont conservées, déplacées.</li>
					<li><strong>Tuez les tous</strong>: <span style="color:red">Attention!</span> Supprime tous les liens avec une expiration avant leur échéance.</li>
				</ul>
				...
				<form method="post">
					...
					<div>
						<select name="expiry_admin_prune_type" size="1" >
							<option value="lazyorblind" >Sélection</option>
							<option value="expired" >Expiré</option>
							<option value="scrub" >Scrub</option>
							<option value="killall">Tuez les tous</option>
						</select>

						<label>
							<input name="expiry_admin_prune_do" type="hidden" value="false" />
							<input name="expiry_admin_prune_do" type="checkbox" value="true" > Confirmer? 
							<p>Assurez-vous d'avoir fait le bon choix, puis cochez la case.</p>
						</label>
					</div>
			...
			<div  id="stat_tab_api" class="tab">
				...
				<p>Le greffon acceptera les requêtes GET et POST au point de terminaison normal de l’API YOURLS afin de:</p>
				<ul>
					<li>Ajouter les données d'expiration à une nouvelle URL courte</li>
					<li>Ajouter les données d'expiration à une ancienne URL courte</li>
					<li>Récupérer les données d'expiration pour une URL courte</li>
					<li>Nettoyer la base de données en fonction des données d'expiration</li>
				</ul>
				<div>
					<h3>Définition des données d'expiration sur une nouvelle URL courte</h3>
					<p>Avec la requête normale d'API <code>action = "shorturl"</code>:</p>
			
					<p>Pour une expiration basée sur le <strong>nombre de clics</strong>:</p>
					<ul>
					...
					<p>Pour une expiration basée sur le <strong>temps</strong>:</p>
					<ul>
						<li>expiry = "clock"</li>
					...
					<p>Si <code>count</code>, <code>age</code>, and <code>ageMod</code> ne sont pas définies, les valeurs par défaut seront utilisées.</p> 
					<p><strong>Optionel</strong>: définir une URL de post-expiration:</p>
					<ul>
						<li><code>postx = URL</code></li>
					</ul>
				</div>
					<div>
					<h3>Définition des données d'expiration sur une ancienne URL</h3>
					<p>Cette action requiert une authentification. Avec les valeurs de connexion, et en utilisant les mêmes paramètres ci-dessus, envoyez ceci:</p> 
					<ul>
				...
				<div>
					<h3>Récupérer les données d'expiration pour toute URL courte</h3>
					<p>Envoyez la nouvelle action:</p>
					<ul>
				...
				<div>
					<h3>Nettoyer la base de données</h3>
					<p>Ces actions requièrent une authentification. Avec les valeurs de connexion, envoyez ceci:</p> 
					...
					<p>Regardez l'onglet Nettoyer pour d'autres explications</p>

					<h4>example Cron:</h3>
					<p>Utilisez l'exemple pré-construit pour mettre en place un cron quotidien. Cela permet de débarasser la base de données des liens expirés:</p>
					 <iframe src="data:text/html;charset=utf-8,$cronEG" width="100%" height="51"/></iframe>
					<p>Regardez ici pour plus d'infos sur <a href="https://help.ubuntu.com/community/CronHowto" target="_blank" >cron</a> et <a href="https://www.gnu.org/software/wget/manual/html_node/HTTP-Options.html" target="_blank">wget</a>.</p>
				</div>
			</div>
		</div>
	</div>
		...
		<table id="main_table" class="tblSorter" border="1" cellpadding="5" style="border-collapse: collapse">
			<thead>
				<tr>
					...
					<th>Destination PostX (optionel)</th>
				...
				<tr>
					...
					<td>
						<select name="expiry" id="expiry" size="1" >
							<option value="none" selected="selected" >Sélection</option>
							<option value="click">compteur de clics</option>
							<option value="clock">compte à rebours</option>
						</select>
					</td>
...
function expiry_flush() {
	if( isset( $_POST['expiry_admin_prune_do'] ) ) {
		if( $_POST['expiry_admin_prune_do'] == 'true' ) {
			...
			switch ($type) {
				case 'expired':
					echo '<font color="green">Tous les liens expirés ont été retirés.</font>';
					break;
				case 'scrub':
					echo '<font color="green">Tous les liens sont maintenant non périssables.</font>';
					break;
				case 'killall':
					echo '<font color="green">Tous les liens avec une date d"expiration ont été supprimés.</font>';
					break;
				case 'lazyorblind';
					echo '<font color="red">Votre choix manque de clareté... recommencez.</font>';
					break;
				default:
					echo '<font color="red">Un problème est survenu, vérifiez votre base de données.</font>';
			}
		} else {
			echo '<font color="red">Vous avez décider de nettoyer la base de données sans cocher "Confirmer ?"</font>';
		}
	}
}
