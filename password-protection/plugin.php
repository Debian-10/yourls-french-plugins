...
function warning_redirection( $args ) {		
	...
	if ($matthew_pwprotection_array === false) {
		...
		$matthew_pwprotection_array = json_decode(yourls_get_option('matthew_pwprotection'), true);
		if ($matthew_pwprotection_array === false) {
			die("Impossible d'activer la protection par mot de passe. Probablement un soucis de base de données.");
		}
	}
	...
	if( array_key_exists( $matthew_pwprotection_short, $matthew_pwprotection_array ) ){
		if( isset( $_POST[ 'password' ] ) && $_POST[ 'password' ] == $matthew_pwprotection_array[ $matthew_pwprotection_short ] ){ //Check if password is submited, and if it matches the DB
			...
			$error = ( isset( $_POST[ 'password' ] ) ? "<script>alertify.error(\"Mot de passe incorrect, réessayez ; Incorrect pass, please retry\")</script>" : "");
			$matthew_ppu =    yourls__( "URL Protégée/Protected", "matthew_pwp" ); //Translate Password Title
			$matthew_ph =     yourls__( "Mot de passe/Password", "matthew_pwp" ); //Translate the word Password
			$matthew_sm =     yourls__( "Veuillez entrer le mot de passe pour continuer/Enter Pass", "matthew_pwp" ); //Translate the main message
			$matthew_submit = yourls__( "Envoyer! Send!", "matthew_pwp" ); //Translate the Submit button
...
function matthew_pwprotection_display_panel() {
	yourls_register_plugin_page( 'matthew_pwp', 'Protection par mot de passe', 'matthew_pwprotection_display_page' );
}
...
function matthew_pwprotection_process_display() {
	...
	$matthew_su = yourls__( "URL courte"   , "matthew_pwp" ); //Translate "Short URL"
	$matthew_ou = yourls__( "URL originale", "matthew_pwp" ); //Translate "Original URL"
	$matthew_pw = yourls__( "Mot de passe"    , "matthew_pwp" ); //Translate "Password"
	echo <<<TB
	<style>
	table {
