			...
			<div id="tabs">
				...
				<div class="wrap_unfloat">
					<ul id="headers" class="toggle_display stat_tab">
						<li class="selected"><a href="#stat_tab_options"><h2>Options</h2></a></li>
						<li class="selected"><a href="#stat_tab_sub_options"><h2>Obfuscation sous-domaines</h2></a></li>
						<li style="display:$vis_p;"><a href="#stat_tab_toolbox"><h2>Outils</h2></a></li>
						<li style="display:$vis_p;"><a href="#stat_tab_pc"><h2>Compteur de passages</h2></a></li>
					</ul>
				</div>
				...
				<div id="stat_tab_options" class="tab">
					...
					<h3>Obfuscation url courte</h3>
					
					<p>Le comportement par défaut est de retirer les referrer. Optionellement, il peut agir sur tous les liens.</p>
					...
					<form method="post">
						...
						<div class="checkbox">
						  <label>
						    ...
						    <input name="rscrub_scope" type="checkbox" value="all" $scope_chk > Obfuscation pour tous les liens?
						  </label>
						</div>
			
						<div style="display:$vis_some;">	
							<p>L'obfuscation avec condition utilise un suffixe à la fin du lien.</p> 
							<p><strong>Example:</strong> <code>https://$h/$o[1]&#173;V</code>  va obfuscer le lien avec  "V", lorsque <code>https://$h/V</code> est le chemin standard sans modification.</p>
							<p>Définir un autre suffixe, par défaut '@'</p>
							<p>
								<label for="rscrub_prefix">Préfixe trigger </label> 
								<input type="text" id="rscrub_prefix" name="rscrub_prefix" value="$o[1]" /> <small>Changer le paramètre devrait casser rscrub</small>
							</p>
						</div>
						
						<hr>
						
						<h3>Compteur de passages</h3>
						
						<p>Rscrub peut travailler sur des liens non raccourcis en rajoutant un préfixe avec "Prefix and Shorten" en utilisant le même préfixe que les liens courts.</p>
						<p><strong>Example:</strong> <code>https://$h/$o[1]&#173;https://example.com</code> va obfuscer le lien mais pas le raccourcir.</p>
						<div class="checkbox">
						  <label>
						    <input type="hidden" name="rscrub_passthrough" value="false" />
						    <input name="rscrub_passthrough" type="checkbox" value="true" $p_chk > Autoriser l'obfuscation à travers les liens?
						  </label>
						</div>
					...
					<p><strong>Note:</strong> La prévisualisation et les informations d identification ne s affichent pas lors de la publication de liens supprimés sur certains sites de réseaux sociaux. Ce problème peut être résolu: si le plug-in Snapshot Visual Preview est installé, correctement configuré et activé, Rscrub fournit automatiquement un aperçu de la capture instantanée et les en-têtes appropriés à ces sites. Veuillez consulter la page du projet Snapshot sur github pour plus d'informations.</p>
				</div>
				
				<div id="stat_tab_sub_options" class="tab">
				
					<p>Rscrub peut utiliser des sous-domaines pour obfuscer les URL. Cela nécessite un accès aux fichiers de configuration de votre hôte virtuel et des ajouts importants au fichier YOURLS <code>.htaccess</code>. Lorsque vous utilisez cette option, les paramètres de préfixe sont toujours pertinents.</p>
					
					<div style="display:$vis_p;">
					
						<h3>Obfuscation de l'URL directe: Paramètres du sous-domaine</h3>
						
						<p>Si vous utilisez un sous-domaine pour le nettoyage des longues adresses URL, veuillez définir les options ci-dessous. Les options de préfixe sont toujours pertinentes.</p>
						<p>Notez que cela modifiera de nombreuses valeurs dans la boîte de dialogue Outils et apportera des modifications importantes à <code> rscrub.js </ code>.</p>
						...
						<form class="form-horizontal" id="rscrub_pass_subdomain" name="rscrub_pass_subdomain" method="post">
							<div class="checkbox">
							  <label>
							    ...
							    <input name="rscrub_pass_subdomain_do" type="checkbox" value="true" $sub_chk > Utiliser l'obfuscation du sous-domaine?
							  </label>
							</div>
							
							<p>
								<label for="rscrub_pass_subdomain_is">Sous-domaine direct</label> 
								<input type="text" id="rscrub_pass_subdomain_is" name="rscrub_pass_subdomain_is" value="$o[6]" /> Le sous-domaine d'obfuscation par défaut est <code>anon.$h</code>.
							</p>
