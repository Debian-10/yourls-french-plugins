...
function puns_add_pages() {
        yourls_register_plugin_page( 'puns', 'Vérifier les MàJ des greffons', 'puns_do_page' );
}
		...
		<div id="tabs">
			<div class="wrap_unfloat">
				<ul id="headers" class="toggle_display stat_tab">
					<li><a href="#stat_tab_report"><h2>Rapport</h2></a></li>
					<li><a href="#stat_tab_config"><h2>Configuration</h2></a></li>
					<li><a href="#stat_tab_api"><h2>API</h2></a></li>
				</ul>
			</div>
			<div  id="stat_tab_report" class="tab">
	 			<h3> Mettre à jour le rapport</h3>
				<table id="main_table" class="tblSorter" cellpadding="0" cellspacing="1">
				<thead>
					<tr>
						<th>Nom du greffon</th>
						<th>Version</th>
						<th>Note</th>
						<th>Dernière</th>
					</tr>
...
echo <<<HTML
				...
				<form method="post">
					<label>
							...
							<input name="puns_run_report" type="checkbox" value="true" > Faire un rapport? 
					</label>
					<br>
				...
				<form method="post">
					...
					<div class="checkbox" style="padding-left: 10pt;border-left:1px solid blue;border-bottom:1px solid blue;">
						<label>
							...
							<input name="puns_github_auth" type="checkbox" value="true" $ghAuth > Utiliser l'authentification? 
						</label>
						<p>Lorsque séléctionné, vérifie les mises à jour avec les identifiants fournis, ce qui lève partiellement le plafond de requêtes API. Laissez décoché pour garantir l'anonymat, limité à 50 requêtes par heure.</p>
					</div>
					<br>
					<h4>Identifiants GitHub</h4>
					<div style="padding-left: 10pt;border-left:1px solid blue;border-bottom:1px solid blue;">
						<p><label for="puns_github_user">Utilisateur GitHub</label> <input type="text" size=40 id="puns_github_user" name="puns_github_user" value="$ghU" /></p>
						<p><label for="puns_github_pass">Mot de passe GitHub</label> <input type="password" size=40 id="puns_github_pass" name="puns_github_pass" value="$ghP" /></p>
					</div>
					<br>
					<h3>Notifications administratuer: Email</h3>
					<div class="checkbox" style="padding-left: 10pt;border-left:1px solid blue;border-bottom:1px solid blue;">
						<label>
							<input name="puns_notify_do" type="hidden" value="false" />
							<input name="puns_notify_do" type="checkbox" value="true" $punsNotify > Notifier l'administrateur? 
						</label>
						<p>Lorsque séléctionné, le greffon envoie l'état des versions par Email. Prévu pour être utilisé avec cette API et un cron.</p>
					</div>
					<br>
					<h4>Email administrateur</h4>
					<div style="padding-left: 10pt;border-left:1px solid blue;border-bottom:1px solid blue;">
						<p><label for="puns_notify_email">Email administrateur</label> <input type="text" size=40 id="puns_notify_email" name="puns_notify_email" value="$adminEmail" /></p>
					</div>
					<br>
			...
			<div  id="stat_tab_api" class="tab">
				<p>Le greffon va accepter les requêtes avec l'API standard de YOURLS. Prévu pour un usage avec cron et un système de client email.</p>
			
				<div style="padding-left: 10pt;border-left:1px solid blue;border-bottom:1px solid blue;">
					<h3>Vérification rapide avec l'API</h3>
					<p>Envoyer la nouvelle action:</p>
					<ul>
						<li><code>action = "puns-fast"</code></li>
					</ul>
					<h4>Example cron:</h3>
					<p>Utilisez l'exemple pré-fait:</p>
					 <iframe src="data:text/html;charset=utf-8,$cronEG" width="100%" height="51"/></iframe>
					<p>Regardez ici pour plus d'infos <a href="https://help.ubuntu.com/community/CronHowto" target="_blank" >cron</a> et <a href="https://curl.haxx.se/" target="_blank">curl</a>.</p>
				</div>
			</div>
		</div>
	</div>
...
function puns_cycle(){
	...
	foreach( $plugins as $file=>$plugin ) {
		...
		if ( yourls_is_API() ) {
			...
			switch ($result['code']) {
				case -1: $msg = "<span style=\"color:red\"><strong>MàJ disponible</strong></span>"; break;
				case 0:  $msg = "<span style=\"color:green\">à jour</span>"; break;
				case 1:  $msg = "<span style=\"color:blue\">Devant la dernière version</span>"; break;
				case 2:  $msg = "Ce dépôt n'a pas de version correcte"; break;
				case 3:  $msg = $result['err']; break;
				case 403:$msg = "Refusé!"; break;
				case 404:$msg = "Pas d'étiquettes trouvées"; break;
			}
...
function puns_check($url,$version) {
	if (function_exists('curl_init')) {
		...
		elseif (in_array($host, $gitlab)) $data = puns_gitlab($parse, $version);
			...
			$data['code'] = 3;
			$data['err'] = 'Domaine non supporté: <code>'.$parse['host'].'</code>';
		}
		return $data;
	} else print "Veuillez installer PHP-CURL";
}
/*
 *
 *	Helpers
...
function puns_fast_api() {
	...
	if ($data > 0) {
		...
		if ($opt[3] == 'true') {
			mail ($opt[4], "Status des version des greffons YOURLS ", "Il y a des mises à jour disponibles à  ".YOURLS_SITE, "noreply@".$parse['host']);
		}
		return array(
			'statusCode' => 200,
			'code'		 => 1,
			'simple'     => "Des mises à jour sont disponibles pour un ou plusieurs greffons",
			'message'    => 'update_status: MàJ disponible',
		);
	} else {
		$opt = puns_config();
		$parse = parse_url(YOURLS_SITE);	
		if ($opt[3] == 'true') {
			mail ($opt[4], "Status des versions des greffons YOURLS", "Aucune mise à jour disponible à  ".YOURLS_SITE, "noreply@".$parse['host']);
		}
		return array(
			'statusCode' => 200,
			'code'		 => 0,
			'simple'     => "Actuellement, il n'y a pas de MàJ disponible",
			'message'    => 'update_status: à jour',
		);
	}
}
